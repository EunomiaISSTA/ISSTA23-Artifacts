...
int main(int argc, char **argv)
{
    ...
    cdb["directory"]=cwd;
    cdb["file"]=argv[3];
    cdb["command"]=string("cc -g ") + cwd + "/" + argv[3];
    ...
    for (auto &stub : infos)
    {
        ...
        int pos;
        args a;
        std::tie(fn, pos, file, bl, bc, el, ec, a) = stub;
        ...
        else
        {
            ...
            for (int i = 0; i < a.size(); i++)
                types.push_back(llvm::FunctionType::getInt32Ty(*context));
                ...
                for (auto &I : bb)
                {
                    ...
                    if ((Loc = I.getDebugLoc()))
                    {
                        ...
                        if (bl <= l && l <= el && bc <= c && c <= ec && boost::filesystem::equivalent(boost::filesystem::path(fpath), boost::filesystem::path(file)))
                        {
                            ...
                            for (auto &k : a)
                            {
                                argsref.push_back(I.getOperand(k));
                            }
    ...
    llvm::raw_fd_ostream os(argv[2], err);
    if (!err)
    {
        m->print(os, nullptr);
    }
