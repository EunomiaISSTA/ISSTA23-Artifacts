...
public:
    ...
    bool VisitCallExpr(CallExpr* call) {
        ...
        add(call->getSourceRange(), "call", name);
        auto* coll = new NameCollector();
        for(auto *arg : call->arguments()) coll->TraverseStmt((Stmt*)arg);
        add(call->getSourceRange(), "ause", coll->names);
        return true;
    }
};
                ...
                for(auto &it : mergedinfos) {
                    ...
                    if(s.check() == z3::unsat && it.first.isValid()) {
                        ...
                        res.insert({stub.first, adv.first,
                                    ...
                                    end_loc.getLine(), end_loc.getColumn(), adv.second.second.first});
                    }
                }
                flag = false;
            }
